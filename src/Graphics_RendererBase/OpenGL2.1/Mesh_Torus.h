...
public:
	...
	void Init(RendererBase & renderer, ShaderProgram * shader_program)
	{
		max_radius = 1.1f;
		rings = 32;
		sides = 16;
		...
		renderer.CreateInputLayout(vertex_buffer.format_hash, vertex_format, shader_program);
	}
	...
	void PreRender(RendererBase & renderer, ShaderProgram * shader_program)
	{
		if (!static_data)
		{
			unsigned char * vertex_data = (unsigned char *) renderer.Map(&vertex_buffer, MAP_WRITE_INVALIDATE_ALL);
		...
		renderer.Bind(&vertex_buffer);
		renderer.Bind(&index_buffer);
