...
public:
	...
	void Init(RendererBase & renderer, ShaderProgram * shader_program)
	{
		...
		vertex_format.format_entries[0].name = "POSITION0";
		...
		vertex_format.format_entries[1].name = "NORMAL0";
		...
		vertex_format.format_entries[2].name = "TEXCOORD0";
	...
	void PreRender(RendererBase & renderer)
	{
		...
		renderer.SetInputLayout(vertex_buffer.format_hash, 0);
