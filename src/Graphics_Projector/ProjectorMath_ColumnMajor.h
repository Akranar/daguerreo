...
inline void ProjectorMath<true>::Normalize(float vect[3])
{
...
inline void ProjectorMath<true>::MatrixMultiply4x4(float * destination, const float * source_a, const float * source_b)
{
...
inline void ProjectorMath<true>::ViewMatrix(const float * position, const float * direction, const float * up, float * out_view_matrix)
{
	/*
	...
	*/
	...
	out_view_matrix[1] = up[0];
	out_view_matrix[2] = -direction[0];
	out_view_matrix[3] = 0;

	out_view_matrix[4] = right_direction[1];
	out_view_matrix[5] = up[1];
	out_view_matrix[6] = -direction[1];
	out_view_matrix[7] = 0;

	out_view_matrix[8] = right_direction[2];
	out_view_matrix[9] = up[2];
	out_view_matrix[10] = -direction[2];
	out_view_matrix[11] = 0;

	out_view_matrix[12] = - right_direction[0] * position[0] - right_direction[1] * position[1] - right_direction[2] * position[2];
	out_view_matrix[13] = - up[0] * position[0] - up[1] * position[1] - up[2] * position[2];
	out_view_matrix[14] = + direction[0] * position[0] + direction[1] * position[1] + direction[2] * position[2];
	out_view_matrix[15] = 1;
}

inline void ProjectorMath<true>::LookAtMatrix(const float * position, const float * target, const float * up, float * out_view_matrix)
{
	float direction[3];
...
inline void ProjectorMath<true>::PerspectiveProjectionMatrix(float left, float right, float bottom, float top, float distance_to_near, float distance_to_far, float * out_projection_matrix)
{
	...
	out_projection_matrix[8] = (right + left) / (right - left);
	out_projection_matrix[9] = (top + bottom) / (top - bottom);
	out_projection_matrix[10] = -(distance_to_far + distance_to_near) / (distance_to_far - distance_to_near);
	out_projection_matrix[11] = -1;

	out_projection_matrix[12] = 0;
	out_projection_matrix[13] = 0;
	out_projection_matrix[14] = (- 2 * distance_to_near * distance_to_far) / (distance_to_far - distance_to_near);
	out_projection_matrix[15] = 0;
}

inline void ProjectorMath<true>::PerspectiveProjectionMatrix(float fov, float aspect_ratio, float distance_to_near, float distance_to_far, float * out_projection_matrix)
{
...
inline void ProjectorMath<true>::OrthographicProjectionMatrix(float left, float right, float bottom, float top, float distance_to_near, float distance_to_far, float * out_projection_matrix)
{
	...
	out_projection_matrix[11] = 0;

	out_projection_matrix[12] = -(right + left) / (right - left);
	out_projection_matrix[13] = -(top + bottom) / (top - bottom);
	out_projection_matrix[14] = - (distance_to_far + distance_to_near) / (distance_to_far - distance_to_near);
