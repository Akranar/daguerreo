...
int main()
{
	...
	float *buffer = createMandelbrotImage(width, height, -0.802f, -0.177f, 0.011f, 110);
	if (buffer == NULL) {
		return 1;
	}
	...
	printf("Saving PNG\n");
	int result = writeImage("out.png", width, height, buffer, "This is my test image");
...
float *createMandelbrotImage(int width, int height, float xS, float yS, float rad, int maxIteration)
{
	...
	int xPos, yPos;
	float minMu = (float) maxIteration;
	float maxMu = 0;
	...
	for (yPos=0 ; yPos<height ; yPos++)
	{
		...
		for (xPos=0 ; xPos<width ; xPos++)
		{
			...
			if (iteration < maxIteration) {
				...
				float mu = iteration -  float((log(log((double)modZ))) / log((double)2));
