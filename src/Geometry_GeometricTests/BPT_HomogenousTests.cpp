...
bool BasicPrimitiveTests::TestingSphereAgainstSphere(const BoundingSphere & sphere_a, const BoundingSphere & sphere_b)
{	
	/*
		Two spheres only intersect when the distance of their centers <= the sum of their radii.
		Uses squared values to avoid squared-root computations.
	*/
	return ((sphere_a.GetCenter() - sphere_b.GetCenter()).squaredNorm() <= (sphere_a.GetRadius() + sphere_b.GetRadius()) * (sphere_a.GetRadius() + sphere_b.GetRadius()));
}

bool BasicPrimitiveTests::TestingAABBAgainstAABB(const AABB & aabb_a, const AABB & aabb_b)
{
	/*
		Two AABBs only intersect if they overlap on all 3-axes.
	*/
	if (abs(aabb_a.GetCenter()[0] - aabb_b.GetCenter()[0]) > aabb_a.GetExtent(0) + aabb_b.GetExtent(0)) return false;
	if (abs(aabb_a.GetCenter()[1] - aabb_b.GetCenter()[1]) > aabb_a.GetExtent(1) + aabb_b.GetExtent(1)) return false;
	if (abs(aabb_a.GetCenter()[2] - aabb_b.GetCenter()[2]) > aabb_a.GetExtent(2) + aabb_b.GetExtent(2)) return false;
	return true;
}
bool BasicPrimitiveTests::TestingOBBAgainstOBB(const OBB & obb_a, const OBB & obb_b)
{
	/*
		Separating Axis Test with 15 Axes:
			-> Three orientation axes of A
			-> Three orientation axes of B
	...
	*/
	Eigen::Vector3f AB_AFrame = obb_b.GetCenter() - obb_a.GetCenter();
	AB_AFrame = Eigen::Vector3f(AB_AFrame.dot(obb_a.GetAxis(0)), AB_AFrame.dot(obb_a.GetAxis(1)), AB_AFrame.dot(obb_a.GetAxis(2)));
	...
	*/
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		*/
		...
		{
			B_axes_AFrame.coeffRef(i, j) = obb_a.GetAxis(i).dot(obb_b.GetAxis(j));
		...
		/*
			...
			Test A's Axes {
		*/
		radius_a = obb_a.GetExtent(i);
		radius_b = obb_b.GetExtent(0) * abs_B_axes_AFrame.coeff(i, 0) + obb_b.GetExtent(1) * abs_B_axes_AFrame.coeff(i, 1) + obb_b.GetExtent(2) * abs_B_axes_AFrame.coeff(i, 2);
	...
	*/
	for (int i = 0; i < 3; ++i)
	{
		radius_a = obb_a.GetExtent(0) * abs_B_axes_AFrame.coeff(0, i) + obb_a.GetExtent(1) * abs_B_axes_AFrame.coeff(1, i) + obb_a.GetExtent(2) * abs_B_axes_AFrame.coeff(2, i);
		radius_b = obb_b.GetExtent(i);
	...
	*/
	//A0 x B0
	radius_a = obb_a.GetExtent(1) * abs_B_axes_AFrame.coeff(2, 0) + obb_a.GetExtent(2) * abs_B_axes_AFrame.coeff(1, 0);
	radius_b = obb_b.GetExtent(1) * abs_B_axes_AFrame.coeff(0, 2) + obb_b.GetExtent(2) * abs_B_axes_AFrame.coeff(0, 1);
	if (abs(AB_AFrame[2] * B_axes_AFrame.coeff(1, 0) - AB_AFrame[1] * B_axes_AFrame.coeff(2, 0)) > radius_a + radius_b) return false;
	//A0 x B1
	radius_a = obb_a.GetExtent(1) * abs_B_axes_AFrame.coeff(2, 1) + obb_a.GetExtent(2) * abs_B_axes_AFrame.coeff(1, 1);
	radius_b = obb_b.GetExtent(0) * abs_B_axes_AFrame.coeff(0, 2) + obb_b.GetExtent(2) * abs_B_axes_AFrame.coeff(0, 0);
	if (abs(AB_AFrame[2] * B_axes_AFrame.coeff(1, 1) - AB_AFrame[1] * B_axes_AFrame.coeff(2, 1)) > radius_a + radius_b) return false;
	//A0 x B2
	radius_a = obb_a.GetExtent(1) * abs_B_axes_AFrame.coeff(2, 2) + obb_a.GetExtent(2) * abs_B_axes_AFrame.coeff(1, 2);
	radius_b = obb_b.GetExtent(0) * abs_B_axes_AFrame.coeff(0, 1) + obb_b.GetExtent(1) * abs_B_axes_AFrame.coeff(0, 0);
	if (abs(AB_AFrame[2] * B_axes_AFrame.coeff(1, 2) - AB_AFrame[1] * B_axes_AFrame.coeff(2, 2)) > radius_a + radius_b) return false;

	//A1 x B0
	radius_a = obb_a.GetExtent(0) * abs_B_axes_AFrame.coeff(2, 0) + obb_a.GetExtent(2) * abs_B_axes_AFrame.coeff(0, 0);
	radius_b = obb_b.GetExtent(1) * abs_B_axes_AFrame.coeff(1, 2) + obb_b.GetExtent(2) * abs_B_axes_AFrame.coeff(1, 1);
	if (abs(AB_AFrame[0] * B_axes_AFrame.coeff(2, 0) - AB_AFrame[2] * B_axes_AFrame.coeff(0, 0)) > radius_a + radius_b) return false;
	//A1 x B1
	radius_a = obb_a.GetExtent(0) * abs_B_axes_AFrame.coeff(2, 1) + obb_a.GetExtent(2) * abs_B_axes_AFrame.coeff(0, 1);
	radius_b = obb_b.GetExtent(0) * abs_B_axes_AFrame.coeff(1, 2) + obb_b.GetExtent(2) * abs_B_axes_AFrame.coeff(1, 0);
	if (abs(AB_AFrame[0] * B_axes_AFrame.coeff(2, 1) - AB_AFrame[2] * B_axes_AFrame.coeff(0, 1)) > radius_a + radius_b) return false;
	//A1 x B2
	radius_a = obb_a.GetExtent(0) * abs_B_axes_AFrame.coeff(2, 2) + obb_a.GetExtent(2) * abs_B_axes_AFrame.coeff(0, 2);
	radius_b = obb_b.GetExtent(0) * abs_B_axes_AFrame.coeff(1, 1) + obb_b.GetExtent(1) * abs_B_axes_AFrame.coeff(1, 0);
	if (abs(AB_AFrame[0] * B_axes_AFrame.coeff(2, 2) - AB_AFrame[2] * B_axes_AFrame.coeff(0, 2)) > radius_a + radius_b) return false;

	//A2 x B0
	radius_a = obb_a.GetExtent(0) * abs_B_axes_AFrame.coeff(1, 0) + obb_a.GetExtent(1) * abs_B_axes_AFrame.coeff(0, 0);
	radius_b = obb_b.GetExtent(1) * abs_B_axes_AFrame.coeff(2, 2) + obb_b.GetExtent(2) * abs_B_axes_AFrame.coeff(2, 1);
	if (abs(AB_AFrame[1] * B_axes_AFrame.coeff(0, 0) - AB_AFrame[0] * B_axes_AFrame.coeff(1, 0)) > radius_a + radius_b) return false;
	//A2 x B1
	radius_a = obb_a.GetExtent(0) * abs_B_axes_AFrame.coeff(1, 1) + obb_a.GetExtent(1) * abs_B_axes_AFrame.coeff(0, 1);
	radius_b = obb_b.GetExtent(0) * abs_B_axes_AFrame.coeff(2, 2) + obb_b.GetExtent(2) * abs_B_axes_AFrame.coeff(2, 0);
	if (abs(AB_AFrame[1] * B_axes_AFrame.coeff(0, 1) - AB_AFrame[0] * B_axes_AFrame.coeff(1, 1)) > radius_a + radius_b) return false;
	//A2 x B2
	radius_a = obb_a.GetExtent(0) * abs_B_axes_AFrame.coeff(1, 2) + obb_a.GetExtent(1) * abs_B_axes_AFrame.coeff(0, 2);
	radius_b = obb_b.GetExtent(0) * abs_B_axes_AFrame.coeff(2, 1) + obb_b.GetExtent(1) * abs_B_axes_AFrame.coeff(2, 0);
