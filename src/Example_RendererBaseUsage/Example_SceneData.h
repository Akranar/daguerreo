...
public:
	...
	void Init()
	{
		...
		*/
		...
		camera.SetPerspective(45.0f, 1.0f, 1.0f, 500000.0f);
		camera.LookAt(1.0f, 10.0f, 10.0f, 0, 0, 0, 0, 0, 1);
		camera_controller.SetProjector(&camera, Eigen::Vector3f(0, 1, 0));
		camera_controller.SetSpeed(20);
		light_source = camera;
	...
	void OnViewportUpdate(unsigned int width, unsigned height)
	{
		camera.SetPerspective(45.0f, float(width) / float(height), 1.0f, 500000.0f);
	}
