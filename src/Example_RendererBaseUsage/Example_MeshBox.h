#ifndef _EXAMPLE_MESH_BOX_H_
#define _EXAMPLE_MESH_BOX_H_

#include "ExLib_RendererBase.h"
#include "ExLib_ProceduralMesh.h"


class Example_MeshBox
{
public:
	...
	void Init(RendererBase * _renderer)
	{
		...
		ProceduralBox box;
		box.SetOptions(5000, 5000, 5000, true, true, false, true);

		unsigned int vertex_count = box.GetVertexCount();
		index_count = box.GetTriangleListIndexCount();
		...
		{
			box.GetPositions(initial_vertex_data, byte_stride);
			box.GetNormals(initial_vertex_data + sizeof(float) * 3, byte_stride);
			box.GetTexcoord2D(initial_vertex_data + sizeof(float) * 6, byte_stride);
			box.GetTriangleListIndices(initial_index_data);
