#ifndef _EXAMPLE_MESH_QUAD_UP_Y_H_
#define _EXAMPLE_MESH_QUAD_UP_Y_H_

#include "ExLib_RendererBase.h"
#include "ExLib_ProceduralMesh.h"


class Example_MeshQuadUpY
{
public:
	...
	void Init(RendererBase * _renderer)
	{
		...
		float radius = 40;
		corners[0] = Eigen::Vector3f(-radius, 0, radius); 
		corners[1] = Eigen::Vector3f(radius, 0, radius); 
		corners[2] = Eigen::Vector3f(radius, 0, -radius); 
		corners[3] = Eigen::Vector3f(-radius, 0, -radius);
