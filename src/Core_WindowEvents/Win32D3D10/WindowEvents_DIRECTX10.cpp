...
#include "WindowEvents_DIRECTX10_Sizing.h"

//Event Handling
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam);
#define PushInputEvent(input_event) active_window->GetInputManager().PushEvent(input_event);
#include "KeyMapping_DIRECTX10.h"
#include "WindowEvents_DIRECTX10_ProcessEvents.h"
...
{
	if (!initialized) return;
	render_context->GetPimpl()->GetSwapChain()->Present(data->GetVSync(),0);
}
