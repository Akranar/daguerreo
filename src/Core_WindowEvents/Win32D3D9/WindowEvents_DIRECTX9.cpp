...
#include "WindowEvents_DIRECTX9_Sizing.h"

//Event Handling
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam);
#define PushInputEvent(input_event) active_window->GetInputManager().PushEvent(input_event);
#include "KeyMapping_DIRECTX9.h"
#include "WindowEvents_DIRECTX9_ProcessEvents.h"
...
{
	if (!initialized) return;
	//FIXME render_context->GetPimpl()->GetSwapChain()->Present(data->GetVSync(),0);
	render_context->GetPimpl()->GetDevice()->Present(NULL, NULL, NULL, NULL);
}
